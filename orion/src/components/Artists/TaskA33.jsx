...
const TaskA33 = () => {
  ...
  const [prompt, setPrompt] = useState('');
    ...
    const [taskProgress, setTaskProgress] = useState({ current_task: 3, next_task: 4, total_tasks: 5 });
    ...
    <Container className="mt-5">
      <Row>
        ...
        <h2 className="task-title">Task 3</h2>
          <h4 className="text-secondary">Generate a Vibrant Abstract Pattern.</h4>
        </Col>
      </Row>
      <Row className="mt-4">
        <Col md={8}>
          <Card className='card'>
            <Card.Body className='card-body'>
              <Card.Title style={{color:"white"}}>Generate a Themed Abstract Image.</Card.Title>
              <Card.Text style={{color:"white"}}>
              Within the Chromatic Chambers, the artisans are weaving vibrant abstract patterns to represent the kingdom’s thriving creativity. The Artisan Council challenges you to create a unique and dynamic pattern that embodies the spirit of artistic innovation.
                 </Card.Text>
              <Card.Title style={{color:"white"}}>Objective</Card.Title>
              <Card.Text style={{color:"white"}}>
              Craft a prompt to generate a vibrant abstract pattern, using bold colors and intricate designs. The pattern should evoke energy and creativity, showcasing the kingdom’s artistic prowess.  </Card.Text>
            </Card.Body>
          </Card>
        </Col>
        <Col md={4}>
      ...
      {taskProgress && (
          <Row style={{ margin: "50px" }}>
            ...
            <Col md={2}>
              <Button variant="light" style={{ margin: "20px" }} onClick={() => window.location.href='/TaskA34'}>
                Next Task
              </Button>
            </Col>
          </Row>
